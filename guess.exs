# Необходимо модифицировать код из задания модуля 4 и использовать if, cond, либо case
# в тех случаях, где это оправданно.

# Возможное решение задания модуля 4:
...
defmodule Game do
  ...
  defp gets_and_parse(msg), do: Integer.parse(IO.gets(msg)) |> get_integer

  defp get_integer(:error), do: :error
  defp get_integer({int, _}), do: int
end

Game.start |> IO.inspect
